...
def test_build_instructions_truncates_memory_first():
    settings = Settings()
    logger = DummyLogger()
    room_metadata = {
    ...
    assert "CONTEXTO DE ESTA SESIÓN" in result
...
def test_invalid_total_budget_raises():
    with pytest.raises(ValueError, match="llm_max_input_tokens \\+ llm_max_output_tokens"):
        Settings(llm_max_input_tokens=110, llm_max_output_tokens=20, llm_hard_limit_tokens=120)
